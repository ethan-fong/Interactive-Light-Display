...
SWEEP:		LDR		R0, =LEDR_DIRECTION	// put shifting direction into R2
				...
				B TOOUTSIDE 
TOCENTRE:
	AND R12, R3, #0b10000			// AND with R6 to isolate bit in right half
	CMP	R12, #0b10000				// Check to see if bit in right side is in the middle					// Change the direction of shifting if it is in the middle
	BEQ C_O
	AND R10, R3, #0b11111			// Shift the right half once to the left 
	LSL	R10, R10, #1			
	AND	R11, R3, #0b1111100000		// AND with R6 to isolate bit in left half
	LSR	R11, R11, #1				// Shift the left half once to the right
	ORR	R3, R10, R11
	B DONE_SWEEP
C_O:			
	MOV		R2, #1					// change direction to outside
TOOUTSIDE:		
	AND R12, R3, #1					// AND with R6 to isolate bit in right half
	CMP	R12, #0b00000001			// Check to see if swe side is on the outside
	@ MOVEQ R2, #0					// Change the direction of shifting if it is on the outside
	BEQ O_C					// Branch to prevent shifting if it is in the outside
	AND	R10, R3, #0b11111			// Shift the left half once to the right
	LSR	R10, R10, #1
	AND	R11, R3, #0b1111100000		// Shift the right half once to the left
	LSL	R11, R11, #1
	ORR	R3, R10, R11
	B DONE_SWEEP
...
START_STOP:		LDR		R0, =MPCORE_PRIV_TIMER		// timer base address
				LDR		R1, [R0, #0x8]			// read timer control register				// Read timer control register
